...
export default function Canvas(){
  ...
  const onWheel = useCallback((e: React.WheelEvent) => {
    setCamera((camera: { x: number; y: number; zoom: any; }) => ({
      x: camera.x - e.deltaX,
      y: camera.y - e.deltaY,
      zoom: camera.zoom,
    }));
  ...
  return (
    <div className="flex h-screen w-full">
      <main className="fixed left-0 right-0 h-screen overflow-y-auto">
        ...
        >
          ...
          >
            ...
            >
              {layerIds?.map((layerId: React.Key | null | undefined) => (
                <LayerComponent
                  key={layerId}
                  id={layerId}
      ...
      <ToolsBar
        ...
        setCanvasState={(newState: any) => setState(newState)}
        zoomIn={() => {
          setCamera((camera: { zoom: number; }) => ({ ...camera, zoom: camera.zoom + 0.1 }));
        }}
        zoomOut={() => {
          setCamera((camera: { zoom: number; }) => ({ ...camera, zoom: camera.zoom - 0.1 }));
        }}
